...
const AlertCard = ({ alert, index, isSelected, onSelect }: AlertCardProps) => {
  ...
  return (
    <motion.div initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: index * 0.08 }}>
      <button
        type="button"
        onClick={onSelect}
        className={`w-full text-left rounded-xl border p-3.5 transition-all duration-200 ${cfg.border} ${cfg.bg} ${
          isSelected ? 'ring-2 ring-cyber-teal/50' : 'hover:brightness-110'
        }`}
      >
        <div className="flex items-start gap-3">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="flex items-center gap-3 flex-wrap text-xs font-mono">
              <span className={`flex items-center gap-1 ${cfg.iconColor}`}>
    ...
    <div className="space-y-4 h-full flex flex-col">
      <div className="flex items-center justify-between flex-shrink-0">
        <h2 className="font-display text-white font-semibold text-sm tracking-wide flex items-center gap-2">
          ...
          ACTIVE ALERTS
          {aiAlerts.length > 0 && (
            <span className="w-5 h-5 rounded-full bg-cyber-red text-white text-[10px] font-bold flex items-center justify-center animate-pulse">
              {aiAlerts.length}
            </span>
          )}
        </h2>
      ...
      {err && (
        <div className="text-cyber-red text-xs font-mono bg-cyber-red/10 border border-cyber-red/30 rounded-lg px-3 py-2">
          {err}
        </div>
      )}
