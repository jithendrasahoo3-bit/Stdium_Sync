...
import { Languages, Sparkles, AlertCircle, Copy, Check } from 'lucide-react';
  ...
  return (
    <div className="space-y-4">
      ...
      <AnimatePresence mode="wait">
        {selectedAlert ? (
          <motion.div key="selected" initial={{ opacity: 0, y: -5 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
          ...
          <motion.div key="no-selection" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}>
            <div className="rounded-xl border border-dashed border-glass-border p-4 text-center">
              <p className="font-body text-slate-500 text-sm">&larr; Select an alert from the feed to translate</p>
            </div>
          </motion.div>
        )}
      </AnimatePresence>
            ...
            >
              ...
              <div className="relative z-10">
                ...
                <p className="font-mono text-cyber-purple text-xs">
                  &rarr; {selectedLangData?.label} ({selectedLangData?.nativeName})
                </p>
              </div>
      ...
      <AnimatePresence>
        {translationResult && !isTranslating && (
          <motion.div key="result" initial={{ opacity: 0, y: 15 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}>
            <GlassPanel glow="purple" noPadding className="p-5 space-y-4">
              ...
              <div className="bg-black/30 rounded-xl p-4 border border-cyber-purple/20">
                <p
                  className="font-body text-white text-base leading-relaxed"
